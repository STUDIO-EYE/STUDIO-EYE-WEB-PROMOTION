...
//       cy.get('[data-cy="create_artwork_submit"]').click({ force: true });
//       cy.wait(1000);
//   });
...
//         cy.get(`img[src="${image}"]`).should('exist'); // 이미지가 존재하는지 확인
//       });
//     });
//   });
...
//     cy.get('[data-cy="outro-section"]').within(() => {
//       clientTestImages.forEach((image) => {
//         cy.get(`img[src="${image}"]`).should('not.exist'); // 이미지가 존재하는지 확인
//       });
//       // PAClientTestData.forEach((client) => {
