describe('PP-AboutPage가 정상적으로 표시되는지 확인한다.', () => {
  ...
  it('5. Corp Section의 협력사 소개가 잘 나타나는지 확인한다.', () => {
    ...
    const expectedLogos = [
      'https://studio-eye-gold-bucket.s3.ap-northeast-2.amazonaws.com/corp1.png',
      'https://studio-eye-gold-bucket.s3.ap-northeast-2.amazonaws.com/corp2.png',
    ];
  ...
  it('5. (필수 예외) api 요청 결과가 200일 때 전송 받은 협력사 데이터가 없을 경우 화면에 기본 설정된 데이터가 잘 나타나는지 확인한다.', () => {
    ...
    const defaultLink = 'https://www.netflix.com/browse';

    cy.get('[data-cy="company-image"]')
      .first()
      .within(() => {
        cy.get('img').should('exist').should('have.attr', 'src', defaultLogo);
      });
    ...
    cy.get('@windowOpen').should('be.calledWith', defaultLink);
  });
