describe('PP-AboutPage가 정상적으로 표시되는지 확인한다.', () => {
  ...
  it('5. Corp Section의 협력사 소개가 잘 나타나는지 확인한다.', () => {
    ...
    cy.get('[data-cy="company-image"]').each((item, index) => {
      ...
      cy.wrap(item).find('img').should('have.attr', 'src', expectedLogos[index]); // 정확한 src 확인
    });
