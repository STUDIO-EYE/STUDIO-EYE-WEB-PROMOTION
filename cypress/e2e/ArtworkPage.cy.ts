...
//   it('PA 페이지에서 로딩 확인 후 아트워크가 있는 것을 확인한다.',()=>{
...
describe('Artwork-아트워크를 만들고, 만든 아트워크를 확인한다.',()=>{
  ...
  // it('PA 페이지에서 새로운 아트워크를 추가하나, 안 채운 값이 있을 경우.',()=>{
  ...
  it('PA 페이지에서 새로운 아트워크를 추가하나, 링크가 잘못됐을 경우.',()=>{
    cy.visit('/promotion-admin/artwork')
    cy.contains('아트워크 생성하기').click();
    cy.wait(100);
    cy.get('[data-cy="PP_artwork_createBox"]').within(()=>{
  ...
  // it('PA 페이지에서 추가한 아트워크가 정상적으로 나타나는지 확인한다.',()=>{
  //   cy.visit('/promotion-admin/artwork');
  ...
  // it('PP 페이지에서 추가한 아트워크가 정상적으로 나타나는지 확인한다.',()=>{
  //   cy.visit('/artwork')
